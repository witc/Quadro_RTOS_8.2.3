 ...
 * traceDemoApp.c
 *
 * Demo application used by Tracealyzer demo project.
 *
 * Terms of Use
...
void doSomeWork(uint32_t ms)
{
	...
	while (ms > 0)
	{
		portTickType new = xTaskGetTickCount();
		if (old < new)
		{
...
static portTASK_FUNCTION( trcDemoTaskSensor, pvParameters )
{
	...
	traceLabel sensorUserEventLabel;
	...
	sensorUserEventLabel = xTraceOpenLabel(name);
	...
	for( ;; )
	{
		...
		vTracePrintF(sensorUserEventLabel , "Sending msg %d", sensorReading.value);

		if (xQueueSend(sensorQ, &sensorReading, 1) != pdTRUE)
		{
			vTraceUserEvent(xTraceOpenLabel("Sensor queue full. Sample dropped!"));
		}
...
static portTASK_FUNCTION( trcDemoISR, pvParameters )
{
	...
	/* Initialize xNextWakeTime - this only needs to be done once. */
	xNextWakeTime = xTaskGetTickCount();
	vTraceSetISRProperties(1, "ISRTimer1", 3);
	vTraceSetISRProperties(2, "ISRTimer2", 2);
	vTraceSetISRProperties(3, "ISRTimer3", 1);
...
static portTASK_FUNCTION( trcDemoTaskActuator, pvParameters )
{
	...
	for( ;; )
	{
		...
		{
			vTracePrintF(xTraceOpenLabel("Actuator"), "No data...");
		}
		else
		{
			if (inMessage.code == MSGCODE_CONTROL_VALUE)
			{
				vTracePrintF(xTraceOpenLabel("Actuator Out Signal"), "Out: %d",
							 inMessage.value);
			}
		}
	}
...
int vStartDemoApplication(void)
{
	...
	vTracePrintF(consoleLabel, "A float: %f (should be 1)", (float)1.0);
	vTracePrintF(consoleLabel, "A double: %lf (should be 1)", (double)1.0);
	vTracePrintF(consoleLabel, "A signed 8-bit value: %bd (should be -1)", -1);

	sensorQ = xQueueCreate(15, sizeof(QueueMessage) );
	if( sensorQ == 0 )
	{
		vTraceConsoleMessage("Could not create SensorQ!\n");
		return -1;
	}
	/* This functionality can be easily wrapped in trcKernelPort.h */
	vTraceSetQueueName(sensorQ, "SensorQueue");

	actuatorQ = xQueueCreate(3, sizeof(QueueMessage) );
	if( actuatorQ == 0 )
	{
		vTraceConsoleMessage("Could not create ActuatorQ!\n");
		return -2;
	}
	/* This functionality can be easily wrapped in trcKernelPort.h */
	...
	if( Sem1 == NULL )
	{
		vTraceConsoleMessage("Could not create Sem1!\n");
		return -3;
	}
	/* This functionality can be easily wrapped in trcKernelPort.h */
	...
	if( Sem2 == NULL )
	{
		vTraceConsoleMessage("Could not create Sem2!\n");
		return -4;
	}
	/* This functionality can be easily wrapped in trcKernelPort.h */
	...
	if( Sem3 == NULL )
	{
		vTraceConsoleMessage("Could not create Sem3!\n");
		return -5;
	}
	/* This functionality can be easily wrapped in trcKernelPort.h */
	...
	xTaskCreate( trcDemoTaskSensor, "SensorX",
				configMINIMAL_STACK_SIZE*3, &sensorTaskID1, 8, &objectHandle );
	...
	xTaskCreate( trcDemoTaskSensor, "SensorY",
				configMINIMAL_STACK_SIZE*3, &sensorTaskID2, 7, &objectHandle );
	if (objectHandle == NULL)
	{
		return -7;
	}
	...
	xTaskCreate( trcDemoTaskSensor, "SensorZ",
				configMINIMAL_STACK_SIZE*3, &sensorTaskID3, 6, &objectHandle );
	if (objectHandle == NULL)
	{
		return -8;
	}
	...
	xTaskCreate( trcDemoTaskController, "Control",
				configMINIMAL_STACK_SIZE*3, NULL, 4, &objectHandle );
	if (objectHandle == NULL)
	{
		return -9;
	}
	...
	xTaskCreate( trcDemoTaskActuator, "Actuator",
				configMINIMAL_STACK_SIZE*3, NULL, 5, &objectHandle );
	if (objectHandle == NULL)
	{
		return -10;
	}
	...
	xTaskCreate( trcDemoTaskSuperv, "Superv",
				configMINIMAL_STACK_SIZE*3, NULL, 3, &objectHandle );
	if (objectHandle == NULL)
	{
		return -11;
	}
	
	vTaskCreateNotTraced(trcDemoISR, "FakeISR", configMINIMAL_STACK_SIZE*4, NULL, 9);
